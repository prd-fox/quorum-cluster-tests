...
const makeTestParameters = (readTestsToRun, batchAccountsToPreset, batchSize, positiveReadPercentage, readBatchSize) => {
    return {
        readTestsToRun: readTestsToRun,
...
const makeAllTestCombinations = (readTestsToRun, batchAccountsToPreset, batchSize, positiveReadPercentage, readBatchSize) => {
    let parameterSet = [];

    for (let a = 0; a < readTestsToRun.length; a++) {
            ...
            for (let c = 0; c < batchSize.length; c++) {
                for (let d = 0; d < positiveReadPercentage.length; d++) {
                    for (let e = 0; e < readBatchSize.length; e++) {
                        let para = makeTestParameters(readTestsToRun[a], batchAccountsToPreset[b], batchSize[c], positiveReadPercentage[d], readBatchSize[e]);
...
async function whitelistMany(whitelistContract, batchSize, useExisting) {
    let current = [];
    ...
    return whitelistContract.methods.whitelistMany(current)
        .send({from: accAddress, gas: 14000000})
...
async function testIsWhitelisted(whitelistContract, addressList) {
    return whitelistContract.methods.isWhitelistedMany(addressList)
        .send({from: accAddress, gas: 10000000})
        .then(function (receipt) {
...
async function whitelistDuplicates(whitelistContract, testSizes) {
    ...
    let results = [];
    for (let i = 0; i < testSizes.batchAccountsToPreset / maxConcurrent; i++) {
        console.log("Running dup batch " + i + " of " + (testSizes.batchAccountsToPreset / maxConcurrent));
        let promises = [];
        for (let j = 0; j < maxConcurrent; j++) {
            promises.push(whitelistMany(whitelistContract, testSizes.batchSize, true));
        }
...
async function runReadTest(whitelistContract, testSizes) {
    ...
    //deploy preset accounts
    let results = [];
    for (let i = 0; i < testSizes.readTestsToRun / maxConcurrent; i++) {
        console.log("Running reading batch " + i + " of " + (testSizes.readTestsToRun / maxConcurrent));
        let promises = [];
        for (let j = 0; j < maxConcurrent; j++) {
            let addrsToSearch = [];
            ...
            promises.push(testIsWhitelisted(whitelistContract, addrsToSearch));
        }
        let res = await Promise.all(promises);
        results = results.concat(res);
    }
...
async function runDeployFullTest(whitelistContract, testSizes) {
    ...
    let results = [];
    for (let i = 0; i < testSizes.batchAccountsToPreset / maxConcurrent; i++) {
        console.log("Running batch " + i + " of " + (testSizes.batchAccountsToPreset / maxConcurrent));
        let promises = [];
        for (let j = 0; j < maxConcurrent; j++) {
            promises.push(whitelistMany(whitelistContract, testSizes.batchSize, false));
        }
        let res = await Promise.all(promises);
        results = results.concat(res);
    }
...
async function run() {
    let header = [
        "number of read txs", "number of write txs", "number of addresses per write", "% of reads positive", "addresses read per tx",
...
function convertResultToCsv(result) {
    ...
    let timingsReadTime = timings.reading.duration;
    let timingsDeployTime = timings.write.duration;
    let timingsWriteDupTime = timings.writedup.duration;

    return [
        paras.readTestsToRun, paras.batchAccountsToPreset, paras.batchSize, paras.positiveReadPercent, paras.readBatchSize,
