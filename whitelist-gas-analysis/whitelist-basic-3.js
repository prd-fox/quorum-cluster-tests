...
const testParameters = makeAllTestCombinations(
    [10, 100, 1000, 10000], // number of txns to send doing reading
    [10, 100, 1000], //number of txns to whitelist accounts
...
async function runReadTest(whitelistContract, testSizes) {
    ...
    let results = [];
    let promises = [];
    for (let i = 0; i < testSizes.readTestsToRun; i++) {
        console.log("Running reading batch " + i + " of " + (testSizes.readTestsToRun));
        let addrsToSearch = [];
...
async function run() {
    ...
    for (let i = 0; i < testParameters.length; i++) {
        ...
        testResults[i]["timings"] = {
            reading: JSON.parse(await web3.eth.getExecutionResults(testParameters[i].randomId + "-read")),
            write: JSON.parse(await web3.eth.getExecutionResults(testParameters[i].randomId + "-write")),
            writedup: JSON.parse(await web3.eth.getExecutionResults(testParameters[i].randomId + "-writedup")),
        }
